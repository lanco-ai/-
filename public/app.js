...
function clearSession(){clearMorningState();clearImportedPhotos();clearTimeout(draftTimer);draftDirty=false;state={user:null,csrf:null,policy:state.policy,profile:{},draft:{},appointments:[],favorites:[],slots:[],teachers:[]};daily=null;records=[];posts=[];comments=[];currentPost=null;recordNext=postNext=commentNext=null;resetSignature();}
...
function growth(){const r=daily;return head('成长的每一刻','真实的照护日常，由负责宝宝的老师记录。','service')+morningSummary()+`<div class="split"><section class="card"><div id="growth-list">${records.length?records.map(growthCard).join(''):empty('暂时还没有成长记录，老师发布后即可查看。')}</div>${recordNext?'<button class="btn secondary" data-action="more-growth">查看更多记录</button>':''}</section><aside>${[['heart','今日饮食',r?.diet],['clock','今日午睡',r?.nap],['smile','今日情绪',r?.mood]].map(([i,t,v])=>`<div class="card stat"><h3>${icon(i)} ${t}</h3><p>${esc(v||'暂无记录')}</p>${r?`<p class="tiny">${fmt(r.occurred_at)}</p>`:''}</div>`).join('')}</aside></div>`;}
...
function staffPage(tab='appointments'){
  ...
  const tabs=admin?{appointments:'预约管理',growth:'发布成长记录',morning:'晨检记录',slots:'时段与名额',teachers:'老师账号',policies:'正式协议'}:{appointments:'负责的预约',growth:'发布成长记录',morning:'晨检记录'};
  let content='';
...
async function render(){
  ...
  try{
    if(!['login','register'].includes(page)){
      ...
      if(page==='growth'||page==='staff'&&parts[1]==='morning'){const m=await api('/morning');if(seq!==generation)return;morningRecords=m.records;}
