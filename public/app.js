...
function clearSession(){clearImportedPhotos();clearTimeout(draftTimer);draftDirty=false;state={user:null,csrf:null,policy:state.policy,profile:{},draft:{},appointments:[],favorites:[],slots:[],teachers:[]};daily=null;records=[];posts=[];comments=[];currentPost=null;recordNext=postNext=commentNext=null;resetSignature();}
...
function staffPage(tab='appointments'){
  ...
  else if(tab==='policies'&&admin)content=`<p class="tiny" style="margin-bottom:20px">发布后形成新版本，不会覆盖已签署的历史文件。请填入机构审核通过的正式文本。</p><button class="btn secondary" data-action="load-material-policies">载入文档中的两份候选协议</button><div id="policy-review-notes" class="note">请核对公益入户服务适用范围、拍摄限制与测评采集范围。教师在预约后分配，签署时显示待分配。</div><form id="policy-form">${field('organization','服务机构名称','text',state.policy?.organization||'')}${field('contact','机构联系及隐私事务方式','text',state.policy?.contact||'')}${AGREEMENT_NAMES.map((t,i)=>`<div class="field"><label for="policy-${i}">${t} *</label><textarea id="policy-${i}" required minlength="30" maxlength="20000" rows="10">${esc(state.policy?.documents[i]?.text||'')}</textarea></div>`).join('')}<label class="check"><input type="checkbox" id="policy-reviewed" required>我已核对上述事项，确认这些文件已由机构审核，可以向家长发布。</label><div class="form-actions"><button class="btn">发布正式协议版本</button></div></form>`;
...
async function render(){
  clearImportedPhotos();const seq=++generation;let parts=location.hash.replace(/^#\/?/,'').split('/'),page=parts[0]||(state.user?'home':'login');
...
function bindForms(){
  ...
  handleForm('#policy-form',async()=>{await api('/admin/policies','POST',{organization:$('#organization').value,contact:$('#contact').value,reviewAcknowledged:$('#policy-reviewed').checked,documents:AGREEMENT_NAMES.map((_,i)=>({text:$('#policy-'+i).value}))});await render();toast('正式协议新版本已发布');});
  if($('#growth-form')){
