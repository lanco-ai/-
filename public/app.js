...
function resetSignature(){signData='';hasSignature=false;agreed=false;requestKey=null;agreementIndex=0;}
function clearSession(){clearTimeout(draftTimer);draftDirty=false;state={user:null,csrf:null,policy:state.policy,profile:{},draft:{},appointments:[],favorites:[],slots:[],teachers:[]};daily=null;records=[];posts=[];comments=[];currentPost=null;recordNext=postNext=commentNext=null;resetSignature();}
function shell(content,page){
  const active=['profile','booking','agreements','success','growth'].includes(page)?'service':page==='article'?'knowledge':page==='resource'?'resources':page==='post'?'community':page;
  return `<header class="topbar"><div class="navwrap">${brand()}<nav class="nav" aria-label="主导航">${Object.entries(paths).map(([r,t])=>`<a class="${active===r?'active':''}" href="#/${r}">${t}</a>`).join('')}</nav><a href="#/${staff()?'staff':'personal'}" class="account"><span class="avatar">${esc(state.user.name[0])}</span><span>${staff()?'工作台':'我的托育'}</span></a><button class="menu" aria-label="展开导航" data-action="menu">☰</button></div></header><main class="container">${content}</main><footer class="footer"><span>近邻托育 · 用心陪伴每一步成长</span><span><button class="textbtn" data-action="privacy">隐私与服务协议</button> · <button class="textbtn" data-action="password">修改密码</button> · <button class="textbtn" data-action="logout">退出登录</button></span></footer>`;
}
function login(register=false){return `<main class="login"><div class="login-visual"><img src="assets/childcare.png" alt="托育阅读主题配图">${brand()}<div class="login-copy"><div class="eyebrow" style="color:#d4e6cf">CARE CLOSE TO HOME</div><h1>家门口的<br>科学托育服务</h1><p>让每一份托付，都被温柔接住。</p></div></div><div class="login-panel"><div><span class="iconbox">${icon('heart')}</span><h1>${register?'创建家长账号':'欢迎来到近邻托育'}</h1><p>${register?'注册后填写宝宝档案，开始预约。':'登录你的账号，查看宝宝的每一天。'}</p><form id="auth-form" data-register="${register}" style="margin-top:24px">${register?field('name','显示名称','text',''):''}${field('username','账号','text','')}${field('password','密码','password','')}<p class="tiny">账号为 4–32 位字母、数字或下划线；密码至少 12 位。</p>${register?`<label class="check"><input type="checkbox" id="consent" required>我已阅读并同意隐私协议，了解账户信息的处理方式。</label><button type="button" class="textbtn" data-action="privacy">阅读隐私协议</button>${!state.policy?'<p class="error">机构尚未发布正式协议，暂未开放注册。</p>':''}`:''}<div class="error" id="auth-error" role="alert"></div><button class="btn" ${register&&!state.policy?'disabled':''}>${register?'注册家长账号':'登录'}</button></form><div class="row" style="justify-content:center;margin-top:18px"><a class="textbtn" href="#/${register?'login':'register'}">${register?'已有账号，去登录':'还没有账号？注册'}</a></div><p class="tiny" style="margin-top:18px">忘记密码请联系机构管理员核验身份后重置。${state.policy?`<br>${esc(state.policy.contact)}`:''}</p></div></div></main>`;}
function home(){const latest=records[0];return `<div class="welcome"><div><h1>你好，${esc(state.user.name)} <span style="color:#b7985b">☀</span></h1><p>把安心交给近邻，把美好留给成长。</p></div><div class="date">${new Date().toLocaleDateString('zh-CN',{month:'long',day:'numeric',weekday:'long'})}</div></div><section class="hero"><div class="hero-copy"><div class="eyebrow">A LITTLE CLOSER, A LOT MORE CARE</div><h1>离家近一点，<br>陪伴多一点。</h1><p>家门口的科学托育服务<br>让宝宝自在探索，让家长安心每一天。</p>${btn(staff()?'进入工作台':state.profile.baby?'查看宝宝档案':'填写宝宝档案',staff()?'staff':'profile')}</div><div class="hero-photo"><img src="assets/childcare.png" alt="托育陪伴主题配图"><div class="photo-note">${icon('heart')} 每一个小小成长，都值得被看见</div></div></section><div class="entries">${[['service','calendar','托育服务','安心预约，悉心照护',''],['knowledge','book','育儿知识','科学育儿，从容陪伴','peach'],['resources','folder','资源库','实用工具，随时查阅','blue'],['community','chat','家长交流','分享日常，一起成长','pink']].map(([r,i,t,d,c])=>`<a class="entry" href="#/${r}"><span class="iconbox ${c}">${icon(i)}</span><div><h3>${t}</h3><p>${d}</p></div><span class="chevron">›</span></a>`).join('')}</div><div class="home-bottom"><section><div class="section-head"><h2>成长的每一刻</h2><a class="textbtn" href="#/growth">查看成长记录 →</a></div><div class="card">${latest?`<span class="tag">${esc(latest.baby)} · ${esc(latest.teacher_name)}</span><h3 style="margin:12px 0">${esc(latest.title)}</h3><p>${esc(latest.text.slice(0,120))}</p><p class="tiny" style="margin-top:15px">${fmt(latest.occurred_at)}</p>`:empty('老师发布照护记录后，会在这里显示。')}</div></section><section><div class="section-head"><h2>育儿有方法</h2><a class="textbtn" href="#/knowledge">全部知识 →</a></div><div class="card" style="padding:9px 24px">${[6,4,0].map(i=>`<a class="mini-article" href="#/article/${i}"><span class="mini-cover">${icon(ARTICLES[i].icon)}</span><div><h3>${ARTICLES[i].title}</h3><p>${ARTICLES[i].category} · 阅读文章</p></div></a>`).join('')}</div></section></div>`;}
...
function validateLocal(d,booking){const errors={};for(const[k,l,,r]of fieldDefs)if(r!==false&&!String(d[k]??'').trim())errors[k]='请填写'+l;
  ...
  if(booking&&!d.slotId)errors.slotId='请选择预约时间';return errors;
}
...
function agreementBody(i,d,documents=state.policy?.documents){const doc=documents?.[i];if(!doc)return '<p>机构尚未发布此协议。</p>';
  return `<h2>${esc(doc.title)}</h2><div class="policy-text">${esc(doc.text)}</div>${i===2?`<div class="summary">${summary(d)}</div>${fieldDefs.filter(([k])=>!['baby','age','parent','phone'].includes(k)).map(([k,l])=>`<p><strong>${l}：</strong>${esc(d[k]||'未填写')}</p>`).join('')}`:''}`;
}
function agreements(){if(!state.draft.slotId)return head('请先填写预约信息')+btn('去预约托育','booking');if(!state.policy)return head('机构尚未发布正式协议')+btn('返回托育服务','service');
  return head('确认协议，安心启程','请阅读三份文件，核对信息后完成签名。')+steps(1)+`<div class="agreement-layout"><aside class="card side">${AGREEMENT_NAMES.map((t,i)=>`<button data-agreement="${i}" class="${i===agreementIndex?'active':''}">${String(i+1).padStart(2,'0')}　${t}</button>`).join('')}</aside><div class="card"><div class="tiny">${esc(state.policy.organization)} · 发布于 ${fmt(state.policy.created_at)}</div><div class="agreement-body" id="agreement-body">${agreementBody(agreementIndex,selectedDraft())}</div><label class="check"><input type="checkbox" id="agree" ${agreed?'checked':''}>我已阅读并同意以上三份文件，确认填写的信息准确，并授权机构为本次照护处理必要信息。</label><div class="between" style="margin-top:22px"><h3>家长手写签名</h3><button class="textbtn" data-action="clear-sign">清空签名</button></div><div class="signature"><canvas id="signature" aria-label="家长手写签名板"></canvas><span class="hint" ${hasSignature?'hidden':''}>请在这里签名 · 支持鼠标或触摸</span></div><div class="tiny">签名、协议版本和提交时间将随本次预约一同保存。</div><div class="form-actions">${btn('上一步','booking',true)}<button class="btn" id="submit-appointment" data-action="submit-appointment" ${!agreed||!hasSignature?'disabled':''}>提交预约</button></div></div></div>`;
}
...
function postDetail(){return `<div class="article">${head('家长的分享','','community')}${postCard(currentPost)}<section class="card"><h2>评论 · ${currentPost.commentCount}</h2><div id="comment-list">${comments.length?comments.map(commentCard).join(''):'<p class="tiny" style="margin:20px 0">还没有评论，来说说你的想法吧。</p>'}</div>${commentNext?'<button class="textbtn" data-action="more-comments">查看更早评论</button>':''}<form id="comment-form" style="margin-top:24px"><label for="comment">写下你的回应</label><textarea class="comment" id="comment" required maxlength="1000" rows="3" placeholder="温暖的交流，从一句回应开始"></textarea><div class="form-actions"><button class="btn">发表评论</button></div></form></section></div>`;}
function personal(tab='profile'){const tabs={profile:'宝宝档案',appointments:'我的预约',agreements:'我的协议',growth:'成长记录',favorites:'我的收藏',posts:'我的留言'};let body='';
  ...
  else if(tab==='posts')body=`<div id="post-list">${posts.length?posts.map(postCard).join(''):empty('你的分享，会被保存在这里。','community','去家长交流')}</div>${postNext?'<button class="btn secondary" data-action="more-posts" data-mine="1">查看更多留言</button>':''}`;
...
function staffPage(tab='appointments'){
  ...
  const tabs=admin?{appointments:'预约管理',growth:'发布成长记录',slots:'时段与名额',teachers:'老师账号',policies:'正式协议'}:{appointments:'负责的预约',growth:'发布成长记录'};
  let content='';
  if(tab==='appointments')content=appointmentList(state.appointments);
    ...
    content=eligible.length?`<form id="growth-form"><div class="field"><label for="appointmentId">对应预约 *</label><select id="appointmentId" required>${eligible.map(a=>`<option value="${a.id}">${esc(a.baby)} · ${a.date} ${a.time}</option>`).join('')}</select></div><div class="fields">${field('occurredAt','记录时间（北京时间）','datetime-local','')}${field('title','动态标题','text','')}${field('text','老师记录','textarea','')}${field('diet','今日饮食','textarea','',false)}${field('nap','午睡情况','textarea','',false)}${field('mood','情绪情况','textarea','',false)}</div><div class="field"><label for="media-files">活动照片 / 视频（最多 8 个文件）</label><input id="media-files" type="file" accept="image/png,image/jpeg,image/webp,video/mp4,video/webm" multiple><p class="tiny">图片最大 10MB，视频最大 50MB。媒体仅对对应家长、负责老师和管理员开放。</p></div><div class="error" id="growth-error"></div><div class="tiny" id="upload-progress" role="status"></div><div class="form-actions"><button class="btn">发布成长记录</button></div></form>`:empty('尚无已确认的预约，请先确认预约并分配老师。');
  }else if(tab==='slots'&&admin)content=`<form id="slot-form" class="fields">${field('slot-date','日期','date','')}${field('slot-start','开始时间','time','09:00')}${field('slot-end','结束时间','time','12:00')}${field('slot-capacity','名额','number','5')}<div class="full"><p class="tiny">同一天的时段不重叠；已预约的日期和时间不可修改。</p><button class="btn">开放时段</button></div></form><div class="spacer"></div>${state.slots.map(s=>`<div class="list-row between"><div><h3>${s.date} ${s.start}–${s.end}</h3><p>总名额 ${s.capacity} · 剩余 ${s.remaining} · ${s.enabled?'开放中':'已关闭'}</p></div><button class="btn secondary" data-slot-edit="${s.id}">调整名额 / 状态</button></div>`).join('')||'<p>尚未设置时段。</p>'}`;
  ...
  else if(tab==='policies'&&admin)content=`<p class="tiny" style="margin-bottom:20px">发布后形成新版本，不会覆盖已签署的历史文件。请填入机构审核通过的正式文本。</p><form id="policy-form">${field('organization','服务机构名称','text',state.policy?.organization||'')}${field('contact','机构联系及隐私事务方式','text',state.policy?.contact||'')}${AGREEMENT_NAMES.map((t,i)=>`<div class="field"><label for="policy-${i}">${t} *</label><textarea id="policy-${i}" required minlength="30" maxlength="20000" rows="10">${esc(state.policy?.documents[i]?.text||'')}</textarea></div>`).join('')}<label class="check"><input type="checkbox" required>我确认这些文件已由机构审核，可以向家长发布。</label><div class="form-actions"><button class="btn">发布正式协议版本</button></div></form>`;
  else return notFound();
  return head(admin?'机构管理工作台':'老师工作台',`你好，${esc(state.user.name)}。${admin?'安排时段、老师和正式服务内容。':'查看负责的预约，发布照护记录。'}`)+`<div class="personal"><aside class="card side">${Object.entries(tabs).map(([key,name])=>`<button data-staff="${key}" class="${key===tab?'active':''}">${name}</button>`).join('')}<a class="textbtn" href="#/community">进入家长交流 →</a></aside><section class="card"><h2 style="margin-bottom:24px">${tabs[tab]}</h2>${content}</section></div>`;
}
...
async function render(){
  const seq=++generation;let parts=location.hash.replace(/^#\/?/,'').split('/'),page=parts[0]||(state.user?'home':'login');
  ...
  try{
    if(!['login','register'].includes(page)){
      if(['home','service','personal','staff','growth','booking'].includes(page))await refresh();
      if(['home','growth'].includes(page)){const b=await api('/growth');if(seq!==generation)return;records=b.records;daily=b.daily;recordNext=b.next;}
    ...
    else{const routes={home,service,profile:()=>formPage(false),booking:()=>formPage(true),agreements,success,growth,knowledge,
    ...
    document.title='近邻托育 · '+(({home:'首页',staff:'工作台',booking:'预约托育',growth:'成长记录',community:'家长交流'})[page]||'家门口的科学托育服务');
    window.scrollTo(0,0);bindForms();if($('#signature'))bindSignature();
...
function bindForms(){
  ...
  handleForm('#slot-form',async()=>{await api('/admin/slots','POST',{date:$('#slot-date').value,start:$('#slot-start').value,end:$('#slot-end').value,capacity:Number($('#slot-capacity').value)});await render();toast('预约时段已开放');});
  handleForm('#policy-form',async()=>{await api('/admin/policies','POST',{organization:$('#organization').value,contact:$('#contact').value,documents:AGREEMENT_NAMES.map((_,i)=>({text:$('#policy-'+i).value}))});await render();toast('正式协议新版本已发布');});
  if($('#growth-form')){
    $('#appointmentId').onchange=()=>{const a=state.appointments.find(a=>a.id===$('#appointmentId').value);$('#occurredAt').value=`${a.date}T${a.time.slice(0,5)}`;};$('#appointmentId').onchange();
    handleForm('#growth-form',async()=>{
      const files=[...$('#media-files').files];if(files.length>8)throw new Error('最多上传 8 个媒体文件');
      const data={appointmentId:$('#appointmentId').value,occurredAt:new Date($('#occurredAt').value+'+08:00').toISOString(),title:$('#title').value,text:$('#text').value,diet:$('#diet').value,nap:$('#nap').value,mood:$('#mood').value,media:[]};
...
async function appointmentModal(id){const a=state.appointments.find(a=>a.id===id);if(!a)throw new Error('预约不存在，请刷新');
  modal('预约详情',`<div class="summary">${summary(a)}</div><div class="note">状态：${STATUS[a.status]}${a.teacherName?' · 老师：'+esc(a.teacherName):''}${a.staffNote?`<p>${esc(a.staffNote)}</p>`:''}</div><button class="textbtn" data-signed="${a.id}">查看签署文件与健康信息</button>${staff()?`<form id="appointment-form" style="margin-top:20px">${state.user.role==='admin'&&['pending','confirmed'].includes(a.status)?`<div class="field"><label for="assign-teacher">照护老师</label><select id="assign-teacher"><option value="">请选择老师</option>${state.teachers.filter(t=>t.active).map(t=>`<option value="${t.id}" ${t.id===a.teacherId?'selected':''}>${esc(t.name)}</option>`).join('')}</select></div>`:''}<div class="field"><label for="appointment-status">预约状态</label><select id="appointment-status"><option value="">保持当前状态</option>${({pending:['confirmed','rejected'],confirmed:['completed','cancelled'],completed:[],cancelled:[],rejected:[]})[a.status].map(s=>`<option value="${s}">${STATUS[s]}</option>`).join('')}</select></div>${field('staff-note','处理说明','textarea',a.staffNote,false)}<div class="form-actions"><button class="btn">保存处理结果</button></div></form>`:['pending','confirmed'].includes(a.status)?`<div class="form-actions"><button class="btn secondary" data-cancel-appointment="${a.id}">取消预约</button></div>`:''}`);
...
document.addEventListener('click',async e=>{
  ...
  try{
    ...
    else if(d.action==='submit-appointment'){
      if(b.disabled||!hasSignature||!agreed)return;b.disabled=true;b.textContent='正在提交…';
      try{await flushDraft();requestKey??=crypto.randomUUID();await api('/appointments','POST',{profile:state.draft,slotId:state.draft.slotId,policyId:state.policy.id,signature:signData,agreed,requestKey});state.draft={};draftDirty=false;resetSignature();await refresh();go('success');}
      catch(error){if(error.status===409){resetSignature();await refresh();go('booking');}throw error;}
      finally{if(b.isConnected){b.textContent='提交预约';b.disabled=!hasSignature||!agreed;}}
    }
    ...
    else if(d.signed){const a=await api('/appointments/'+d.signed+'/agreement');modal('已签署文件',`<p>${esc(a.organization)} · ${fmt(a.confirmedAt)}</p>${a.documents.map((_,i)=>`<section class="agreement-body">${agreementBody(i,a.appointment,a.documents)}</section>`).join('')}<h3>家长手写签名</h3><img class="agreement-image" src="${a.signature}" alt="本次预约的家长签名"><p class="tiny break">文件校验摘要：${esc(a.evidenceHash)}</p>`);}
    ...
    else if(d.slotEdit){const s=state.slots.find(s=>s.id===d.slotEdit);modal('调整时段',`<p>${s.date} ${s.start}–${s.end}</p><form id="edit-slot-form">${field('edit-capacity','总名额','number',s.capacity)}<label class="check"><input type="checkbox" id="edit-enabled" ${s.enabled?'checked':''}>开放预约</label><div class="form-actions"><button class="btn">保存</button></div></form>`);handleForm('#edit-slot-form',async()=>{await api('/admin/slots/'+s.id,'PATCH',{capacity:Number($('#edit-capacity').value),enabled:$('#edit-enabled').checked});$('#modal').close();await render();toast('时段已更新');});}
