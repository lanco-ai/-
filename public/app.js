...
function staffPage(tab='appointments'){
  ...
  const tabs=admin?{appointments:'预约管理',growth:'发布成长记录',morning:'晨检记录',slots:'时段与名额',teachers:'老师账号',policies:'正式协议',institution:'机构展示',moderation:'帖子审核'}:{appointments:'负责的预约',growth:'发布成长记录',morning:'晨检记录',moderation:'帖子审核'};
  let content='';
...
async function render(){
  ...
  try{
    if(!['login','register'].includes(page)){
      if(['service','personal','staff','growth','booking'].includes(page))await refresh();
    ...
    else{const routes={home,service,profile:()=>formPage(false),booking:()=>formPage(true),agreements,success,growth,knowledge:()=>knowledge(parts[1]),
