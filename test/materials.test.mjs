...
test('version 4 upgrade leaves legacy signatures and service type unchanged',()=>{
 const dir=mkdtempSync(join(tmpdir(),'jinlin-v4-'));
 try{let db=openDatabase(dir);db.exec(`
  ALTER TABLE growth DROP COLUMN game_activity; DROP TABLE consultation_requests; DROP TABLE consultation_media; DROP TABLE consultation_messages; DROP TABLE consultations; DROP TABLE service_reviews;
  ALTER TABLE slots DROP COLUMN service_type; ALTER TABLE policies DROP COLUMN service_type;
  ALTER TABLE appointments DROP COLUMN service_type; ALTER TABLE appointments DROP COLUMN address; ALTER TABLE appointments DROP COLUMN signatures;
  INSERT INTO users(id,username,name,password,role,created_at) VALUES('u','user','旧家长','hash','parent','2026-01-01');
  ...
  VALUES('a','u','s','{}','p','historical-unified-signature','hash','request','2026-01-01','2026-01-01','[{"text":"历史原文"}]');
  PRAGMA user_version=4;
 `);db.close();db=openDatabase(dir);assert.equal(db.prepare('PRAGMA user_version').get().user_version,6);
 const a=db.prepare('SELECT * FROM appointments').get();assert.equal(a.signature,'historical-unified-signature');assert.equal(a.signatures,null);assert.equal(a.service_type,null);assert.equal(JSON.parse(a.signed_documents)[0].text,'历史原文');assert.equal(db.prepare('SELECT service_type FROM slots').get().service_type,null);db.close();
...
test('version 3 upgrade preserves published and hidden community posts',()=>{
  ...
  try{
    let db=openDatabase(dir);db.exec(`
      ...
      ALTER TABLE growth DROP COLUMN game_activity; DROP TABLE consultation_requests; DROP TABLE consultation_media; DROP TABLE consultation_messages; DROP TABLE consultations; DROP TABLE service_reviews; ALTER TABLE slots DROP COLUMN service_type; ALTER TABLE policies DROP COLUMN service_type; ALTER TABLE appointments DROP COLUMN service_type; ALTER TABLE appointments DROP COLUMN address; ALTER TABLE appointments DROP COLUMN signatures; DROP INDEX posts_moderation; ALTER TABLE posts DROP COLUMN moderation; ALTER TABLE posts DROP COLUMN review_note;
...
test('version 5 upgrade preserves old growth and defaults new game activity to empty',()=>{
 const dir=mkdtempSync(join(tmpdir(),'jinlin-v5-'));
 try{let db=openDatabase(dir);db.exec(`
  ALTER TABLE growth DROP COLUMN game_activity;
  INSERT INTO users(id,username,name,password,role,created_at) VALUES('u','user','旧账号','hash','parent','2026-01-01');
  INSERT INTO policies(id,organization,contact,documents,hash,created_at,created_by) VALUES('p','机构','联系','[]','hash','2026-01-01','u');
  INSERT INTO slots(id,date,start,end,capacity) VALUES('s','2026-01-01','09:00','10:00',1);
  INSERT INTO appointments(id,user_id,slot_id,profile,policy_id,signature,evidence_hash,request_key,created_at,updated_at)
  VALUES('a','u','s','{}','p','historical-signature','hash','request','2026-01-01','2026-01-01');
  INSERT INTO growth(appointment_id,teacher_id,occurred_at,title,text,diet,nap,mood,created_at)
  VALUES('a','u','2026-01-01','旧标题','旧记录','旧饮食','旧午睡','旧情绪','2026-01-01');
  PRAGMA user_version=5;
 `);db.close();db=openDatabase(dir);
 assert.equal(db.prepare('PRAGMA user_version').get().user_version,6);
 const r=db.prepare('SELECT * FROM growth').get();assert.equal(r.game_activity,'');assert.equal(r.text,'旧记录');assert.equal(r.diet,'旧饮食');
