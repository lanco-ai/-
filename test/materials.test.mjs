...
test('version 4 upgrade leaves legacy signatures and service type unchanged',()=>{
 ...
 try{let db=openDatabase(dir);db.exec(`
  DROP TABLE consultation_requests; DROP TABLE consultation_media; DROP TABLE consultation_messages; DROP TABLE consultations; DROP TABLE service_reviews;
 ...
 `);db.close();db=openDatabase(dir);assert.equal(db.prepare('PRAGMA user_version').get().user_version,5);
